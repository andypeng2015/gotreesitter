...
- Release process: grammar tiers are now published with every release
  (`cgo_harness/tier_scan/tiers.md` / `tiers.json`, regenerated by
  `cgo_harness/docker/run_tier_scan.sh`; tier rules in
  `cgo_harness/tier_scan/README.md`). Tier IV = not parity-clean vs the C
  oracle, full stop; tiers I-III rank parity-clean grammars by performance.
  The first tier-publishing release requires tier IV to be empty
