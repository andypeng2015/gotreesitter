...
func normalizeCppMalformedClassFunctionDefinitionRecovery(root *Node, source []byte, lang *Language) {
	if root == nil || lang == nil || lang.Name != "cpp" || root.ownerArena == nil || len(source) == 0 || root.Type(lang) != "ERROR" {
		return
	}
	translationUnitSym, ok := symbolByName(lang, "translation_unit")
	if !ok {
		return
	}
	children := root.children
	for i := range children {
		classSpec, nextIndex, ok := cppClassSpecifierFromRootChildren(root.ownerArena, lang, source, children, i)
		if !ok {
			continue
		}
		functionDef, consumedEnd, ok := cppMalformedClassFunctionDefinitionNode(root.ownerArena, lang, source, classSpec, children)
		if !ok {
			continue
		}
		...
		for _, tail := range children[nextIndex:] {
			if tail != nil && tail.startByte >= consumedEnd {
				out = append(out, tail)
			}
		}
		retagResultRoot(root, translationUnitSym, symbolIsNamed(lang, translationUnitSym))
...
func cppClassSpecifierFromRootChildren(arena *nodeArena, lang *Language, source []byte, children []*Node, index int) (*Node, int, bool) {
	if arena == nil || lang == nil || index < 0 || index >= len(children) || children[index] == nil {
		return nil, index, false
	}
	child := children[index]
	if child.Type(lang) == "ERROR" {
		if classSpec := firstCNodeOfType(child, lang, "class_specifier"); classSpec != nil {
			return classSpec, index + 1, true
		}
	}
	if child.Type(lang) != "ERROR" || index+4 >= len(children) || !cppRecoveredClassKeyword(source, child) {
		return nil, index, false
	}
	className := children[index+1]
	if className == nil || className.Type(lang) != "_class_name" {
		return nil, index, false
	}
	cursor := index + 2
	var baseClause *Node
	if cursor < len(children) && children[cursor] != nil && children[cursor].Type(lang) == "base_class_clause" {
		baseClause = children[cursor]
		cursor++
	}
	if cursor >= len(children) || children[cursor] == nil || children[cursor].Type(lang) != "{" {
		return nil, index, false
	}
	...
	classKeywordSym, ok := symbolByName(lang, "class")
	if !ok {
		return nil, index, false
	}
	classChildren := make([]*Node, 0, 4)
	classChildren = append(classChildren, cLeaf(arena, lang, classKeywordSym, child.startByte, child.endByte, source))
	classChildren = append(classChildren, cppClassNameForRecoveredClass(arena, className))
	if baseClause != nil {
		classChildren = append(classChildren, cloneTreeNodesIntoArena(baseClause, arena))
	}
	classChildren = append(classChildren, fieldList)
	classSpec := newParentNodeInArena(
		arena,
		classSpecSym,
		symbolIsNamed(lang, classSpecSym),
		classChildren,
...
func cppClassNameForRecoveredClass(arena *nodeArena, className *Node) *Node {
	...
	if className.ChildCount() == 1 {
		if child := className.Child(0); child != nil {
			return cloneTreeNodesIntoArena(child, arena)
		}
	}
	return cloneTreeNodesIntoArena(className, arena)
}

func cppRecoveredClassKeyword(source []byte, node *Node) bool {
...
func cppFieldDeclarationListForRecoveredClass(arena *nodeArena, lang *Language, lbrace, fieldList *Node) *Node {
	...
	fieldListSym, ok := symbolByName(lang, "field_declaration_list")
	if !ok {
		return nil
	}
	children := make([]*Node, 0, fieldList.ChildCount()+1)
	children = append(children, cloneTreeNodesIntoArena(lbrace, arena))
	...
	return newParentNodeInArena(arena, fieldListSym, symbolIsNamed(lang, fieldListSym), children, nil, 0)
}

func cppClassSpecifierFieldIDs(arena *nodeArena, lang *Language, count int) []FieldID {
	if count < 2 {
		return nil
	}
	fields := map[int]string{1: "name", count - 1: "body"}
	return cppFieldIDs(arena, lang, count, fields)
}

func cppMalformedClassFunctionDefinitionNode(arena *nodeArena, lang *Language, source []byte, classSpec *Node, oldRootChildren []*Node) (*Node, uint32, bool) {
	if arena == nil || lang == nil || classSpec == nil || int(classSpec.endByte) > len(source) {
		return nil, 0, false
	}
	syms, ok := languageSymbols(
		lang,
		"function_definition",
		"function_declarator",
	...
	functionDefinitionSym := syms[0]
	functionDeclaratorSym := syms[1]
	qualifiedIdentifierSym := syms[2]
	namespaceIdentifierSym := syms[3]
	identifierSym := syms[4]
	parameterListSym := syms[5]
	compoundStatementSym := syms[6]
	lparenSym := syms[7]
	rparenSym := syms[8]
	lbraceSym := syms[9]
	rbraceSym := syms[10]
	...
	voidStart := rustSkipSpaceBytes(source, classSpec.endByte)
	if !cppIdentifierTextAt(source, voidStart, "void") {
		return nil, 0, false
	}
	voidEnd := voidStart + 4
	scopeNameStart := rustSkipSpaceBytes(source, voidEnd)
	scopeNameEnd := cppScanIdentifier(source, scopeNameStart)
	if scopeNameEnd == scopeNameStart || scopeNameEnd+2 > uint32(len(source)) || source[scopeNameEnd] != ':' || source[scopeNameEnd+1] != ':' {
		return nil, 0, false
	}
	nameStart := scopeNameEnd + 2
	nameEnd := cppScanIdentifier(source, nameStart)
	if nameEnd == nameStart {
		return nil, 0, false
	}
	lparen := rustSkipSpaceBytes(source, nameEnd)
	if lparen+1 >= uint32(len(source)) || source[lparen] != '(' || source[lparen+1] != ')' {
		return nil, 0, false
	}
	rparen := lparen + 1
	lbrace := rustSkipSpaceBytes(source, rparen+1)
	if lbrace >= uint32(len(source)) || source[lbrace] != '{' {
		return nil, 0, false
	}
	rbrace, ok := cppFindMatchingBrace(source, lbrace)
	if !ok {
		return nil, 0, false
	}

	classSpecClone := cloneTreeNodesIntoArena(classSpec, arena)
	voidNamespace := cLeaf(arena, lang, namespaceIdentifierSym, voidStart, voidEnd, source)
	scopeIdentifier := cLeaf(arena, lang, identifierSym, scopeNameStart, scopeNameEnd, source)
	scopeError := newParentNodeInArena(arena, errorSymbol, true, []*Node{scopeIdentifier}, nil, 0)
	scopeError.setExtra(true)
	scopeError.setHasError(true)
	nameIdentifier := cLeaf(arena, lang, identifierSym, nameStart, nameEnd, source)

	qualifiedChildren := []*Node{
		voidNamespace,
		scopeError,
		cLeaf(arena, lang, scopeSym, scopeNameEnd, scopeNameEnd+2, source),
		nameIdentifier,
	}
	qualified := newParentNodeInArena(
		arena,
		qualifiedIdentifierSym,
		symbolIsNamed(lang, qualifiedIdentifierSym),
		qualifiedChildren,
		cppFieldIDs(arena, lang, len(qualifiedChildren), map[int]string{0: "scope", 3: "name"}),
		0,
	)
	qualified.setHasError(true)

	parameters := newParentNodeInArena(
		arena,
		parameterListSym,
		symbolIsNamed(lang, parameterListSym),
		[]*Node{
			cLeaf(arena, lang, lparenSym, lparen, lparen+1, source),
			cLeaf(arena, lang, rparenSym, rparen, rparen+1, source),
		},
		nil,
		0,
	)
	declaratorChildren := []*Node{qualified, parameters}
	declarator := newParentNodeInArena(
		arena,
		functionDeclaratorSym,
		symbolIsNamed(lang, functionDeclaratorSym),
		declaratorChildren,
		cppFieldIDs(arena, lang, len(declaratorChildren), map[int]string{0: "declarator", 1: "parameters"}),
		0,
	)
	declarator.setHasError(true)

	compoundChildren := []*Node{cLeaf(arena, lang, lbraceSym, lbrace, lbrace+1, source)}
	if comment := cppCommentChildInRange(arena, lang, source, oldRootChildren, lbrace+1, rbrace); comment != nil {
		compoundChildren = append(compoundChildren, comment)
	}
	compoundChildren = append(compoundChildren, cLeaf(arena, lang, rbraceSym, rbrace, rbrace+1, source))
	compound := newParentNodeInArena(
		arena,
		compoundStatementSym,
		symbolIsNamed(lang, compoundStatementSym),
		compoundChildren,
		nil,
		0,
	)

	functionChildren := []*Node{classSpecClone, declarator, compound}
	functionDef := newParentNodeInArena(
		arena,
		functionDefinitionSym,
		symbolIsNamed(lang, functionDefinitionSym),
		functionChildren,
		cppFieldIDs(arena, lang, len(functionChildren), map[int]string{0: "type", 1: "declarator", 2: "body"}),
		0,
	)
	functionDef.setHasError(true)
	return functionDef, rbrace + 1, true
}

func cppFieldIDs(arena *nodeArena, lang *Language, count int, fields map[int]string) []FieldID {
	if count <= 0 || len(fields) == 0 || lang == nil {
		return nil
	}
	fieldIDs := make([]FieldID, count)
	any := false
	for index, name := range fields {
		if index < 0 || index >= count || name == "" {
			continue
		}
		fieldID, ok := lang.FieldByName(name)
		if !ok {
			continue
		}
...
func cppCommentChildInRange(arena *nodeArena, lang *Language, source []byte, oldRootChildren []*Node, start, end uint32) *Node {
	for _, child := range oldRootChildren {
		if child == nil || child.startByte < start || child.endByte > end || child.Type(lang) != "comment" {
			continue
		}
		cloned := cloneTreeNodesIntoArena(child, arena)
		if cloned != nil {
			cloned.setExtra(true)
		}
		return cloned
	}
	start, end = rustTrimSpaceBounds(source, start, end)
	if node, ok := cRecoverCommentNodeFromRange(source, start, end, lang, arena); ok {
		return node
	}
	return nil
}

func cppIdentifierTextAt(source []byte, start uint32, text string) bool {
	end := start + uint32(len(text))
	if end > uint32(len(source)) || string(source[start:end]) != text {
		return false
	}
	if start > 0 && rustIsIdentByte(source[start-1]) {
		return false
	}
	if end < uint32(len(source)) && rustIsIdentByte(source[end]) {
		return false
	}
	return true
}

func cppScanIdentifier(source []byte, start uint32) uint32 {
	end := start
	for end < uint32(len(source)) && rustIsIdentByte(source[end]) {
		end++
	}
	return end
}

func cppFindMatchingBrace(source []byte, open uint32) (uint32, bool) {
	if open >= uint32(len(source)) || source[open] != '{' {
		return 0, false
	}
	depth := 0
	inString := byte(0)
	escaped := false
	for i := open; i < uint32(len(source)); i++ {
		b := source[i]
		if inString != 0 {
			if escaped {
				escaped = false
...
func firstCNodeOfType(n *Node, lang *Language, typeName string) *Node {
	if n == nil || lang == nil {
		return nil
	}
	...
	for i := 0; i < n.ChildCount(); i++ {
		if found := firstCNodeOfType(n.Child(i), lang, typeName); found != nil {
			return found
		}
	}
	return nil
