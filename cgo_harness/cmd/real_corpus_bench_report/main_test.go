...
func TestBuildReportRanksWorstRatio(t *testing.T) {
	...
	r := buildReport([]string{"bench.txt"}, samples, nil, nil, nil)
...
func TestBuildReportIncludesParityBlockedFailureOnlyLanguage(t *testing.T) {
	...
	r := buildReport([]string{"container.log"}, nil, nil, failures, nil)
	if r.Samples != 0 {
...
func TestBuildReportParityFailureOverridesSamples(t *testing.T) {
	...
	r := buildReport([]string{"sampled.log", "full-source.log"}, samples, nil, failures, nil)
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportParityFailureOverridesRunAbort(t *testing.T) {
	...
	r := buildReport([]string{"metadata.txt", "container.log"}, nil, nil, failures, nil)
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportIncludesBenchmarkRunAbortedLanguage(t *testing.T) {
	...
	r := buildReport([]string{"metadata.txt"}, nil, nil, failures, nil)
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportCompleteMeasurementsOverrideStaleCorpusUnavailableFailure(t *testing.T) {
	...
	r := buildReport([]string{"old/container.log", "new/container.log"}, samples, nil, failures, nil)
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportCompleteMeasurementsOverrideEditCandidateParityFailure(t *testing.T) {
	...
	r := buildReport([]string{"container.log"}, samples, nil, failures, []string{"authzed"})
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportIncludesAttemptedOnlyLanguage(t *testing.T) {
	r := buildReport([]string{"metadata.txt"}, nil, nil, nil, []string{"agda"})
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
func TestBuildReportUsesEditLaneSkipDiagnostic(t *testing.T) {
	...
	r := buildReport([]string{"container.log"}, samples, nil, failures, []string{"json"})
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
`), 0o644)
	...
	r := buildReport([]string{path}, nil, nil, failures, []string{"json"})
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
...
`), 0o644)
	...
	r := buildReport([]string{path}, samples, nil, failures, []string{"twig"})
	if len(r.Languages) != 1 {
		t.Fatalf("languages=%d, want 1", len(r.Languages))
	}
	lang := r.Languages[0]
