...
CORPUS_MAX_ISSUES="${GOT_JAVA_CORPUS_MAX_ISSUES:-}"
TIMEOUT_SWEEP="${GOT_JAVA_TIMEOUT_SWEEP:-}"
PARSE_MODES="${GOT_JAVA_PARSE_MODES:-}"
BENCH_TIMEOUT="${GOT_JAVA_BENCH_TIMEOUT:-}"
BENCH_REGEX='BenchmarkJavaCorpus(GoTreeSitterParseDFA|GoTreeSitterParseDFANoCompat|GoTreeSitterParseTokenSource|GoTreeSitterParseAspectFallback|CTreeSitterParseFull)$'
BENCH_COUNT="10"
BENCH_TIME="750ms"
GOMAXPROCS_VALUE="1"
...
Java corpus knobs:
  ...
  --max-issues <n>            GOT_JAVA_CORPUS_MAX_ISSUES captured in timeout logs
                              (default inside the Go harness: 10)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --max-issues) CORPUS_MAX_ISSUES="$2"; shift 2 ;;
...
require_non_negative_int "--max-issues" "$CORPUS_MAX_ISSUES"
...
[[ -n "$CORPUS_MAX_ISSUES" ]] && ENV_ARGS+=(-e "GOT_JAVA_CORPUS_MAX_ISSUES=$CORPUS_MAX_ISSUES")
...
for var in GOTOOLCHAIN GOT_PARSE_MEMORY_BUDGET_MB GOT_GLR_MAX_MERGE_PER_KEY GOT_GLR_MAX_STACKS GOT_PARSE_NODE_LIMIT_SCALE GOT_GLR_FORCE_CONFLICT_WIDTH GOT_GLR_V2_COMPACT_FULL_LEAVES GOT_GLR_V2_PENDING_PARENTS GOT_GLR_V2_FINAL_CHILD_REFS GOT_JAVA_AMBIGUITY_PROFILE GOT_JAVA_RUNTIME_AUDIT GOT_JAVA_TREE_SHAPE GOT_JAVA_PERF_STATS; do
  if [[ -n "${!var:-}" ]]; then
    ENV_ARGS+=("-e" "$var=${!var}")
  fi
done
...

{
  ...
  echo "glr_v2_compact_full_leaves=${GOT_GLR_V2_COMPACT_FULL_LEAVES:-inherit}"
  echo "glr_v2_pending_parents=${GOT_GLR_V2_PENDING_PARENTS:-inherit}"
