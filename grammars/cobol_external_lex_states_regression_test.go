...
func TestCobolExternalLexStatesRegression(t *testing.T) {
	want := [][]bool{
	...
	if !diag.Supported {
		t.Fatalf("DiagnoseCRecoveryGate rejected cobol with staged ELS: %s", diag.Reason)
	}
	if !lang.CRecoveryCostCompetitionCapable || !lang.CRecoveryCostCompetitionEnabledByDefault {
		t.Fatalf("cobol C recovery election did not enable under cobol_precise_els: capable=%v default=%v",
